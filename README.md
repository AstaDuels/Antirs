--[[ ASTA REMAKE – Full Hub (Simplificado) Speed Booster (LinearVelocity) integrado ao Carry Speed e Lagger. Botões móveis individuais + fundo personalizado. Botão de Lock na barra superior. Reset aprimorado (substitui Insta Reset e Medusa Reset). Bypass TP + Anti Die. Drop com ApplyImpulse. SEM AIMBOT. 
     ANTI-RAGDOLL SIMPLES (forceReset com cooldown) – substitui a versão do Pulse.
]]
...
local function CandyApplyCustomSky(mode)
    ...
    if preset.sky then local skyInst=Instance.new("Sky");skyInst:SetAttribute(CANDY_SKY_TAG,true)
    ...
    if preset.atm then local atm=Instance.new("Atmosphere");atm:SetAttribute(CANDY_SKY_TAG,true)
    ...
    if preset.clouds and terrain then local clouds=Instance.new("Clouds");clouds:SetAttribute(CANDY_SKY_TAG,true)
...
-- ============================================================ -- SPEED BOOSTER (LinearVelocity) – CORRIGIDO PARA NÃO TRAVAR -- ============================================================
...
-- ============================================================ -- ANTI-RAGDOLL SIMPLES (fornecido pelo usuário) -- ============================================================
...
local function ResetPlayer()
    ...
    if not character then resetCooldownGlobal = false return end
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if not humanoid then resetCooldownGlobal = false return end
...
LP.CharacterAdded:Connect(function() StopResetSequence() resetCooldownGlobal = false currentCharacter = nil resetSuccessful = false stopResetSequence = false end)
...
-- ============================================================ -- ANTIRAGDOLL, INFJUMP, ETC (mantido) -- ============================================================
...
local function onHealthChanged(newHealth)
    ...
    if newHealth <= 5 and antiDieHumanoid and antiDieHumanoid.Health > 0 then
        antiDieHumanoid.Health = antiDieHumanoid.MaxHealth return
    end
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    pcall(function() local cg=game:GetService("CoreGui");local old=cg:FindFirstChild(guiName);if old then old:Destroy() end
        local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild(guiName);if o then o:Destroy() end end end)
...
-- ============================================================ -- FUNÇÕES DE TRIGGER DE RAGDOLL (para GUI) -- ============================================================
...
local function getAutoPathSpeed()
    if laggerModeEnabled then return LAGGER_SPEED else return NS end
end
...
enableStretchRez=function()
    ...
    pcall(function() RunService:BindToRenderStep(STRETCH_NAME,Enum.RenderPriority.Last.Value-1,function()
...
enableAntiLag=function()
    ...
    for _,e in pairs(Lighting:GetChildren()) do
        pcall(function() if e:IsA("BlurEffect") or e:IsA("SunRaysEffect") or e:IsA("ColorCorrectionEffect") or e:IsA("BloomEffect") or e:IsA("DepthOfFieldEffect") then e.Enabled=false end end)
    end
...
-- ============================================================ -- SAVE CONFIG (atualizado) -- ============================================================
saveConfig = function()
    local function ks(e) if e.kb then return {kb = e.kb.Name, gp = e.gp and e.gp.Name} elseif e.gp then return {gp = e.gp.Name} else return {kb = nil, gp = nil} end end
    local cfg = {
        ...
        resetKey = ks(KB.Reset), guiHideKey = ks(KB.GuiHide),
        ...
        autoSwing = autoSwingEnabled, unwalkEnabled = unwalkEnabled,
        ...
        bypassTPEnabled = bypassTPEnabled, antiDieEnabled = antiDieEnabled,
        dropEnabled = dropEnabled, resetKeybind = resetKeybind.Name
    }
...
local function resetAllSettings()
    ...
    autoSwitchSpeedEnabled=false;antiRagdollEnabled=false
...
local function createStealBar()
    ...
    task.spawn(function() local t=0 while sbStroke and sbStroke.Parent do t=t+0.05 sbStroke.Transparency=0.2+math.abs(math.sin(t*2))*0.35 sbStroke.Color=Color3.fromRGB(255,255,255) task.wait(0.04) end end)
...
-- ============================================================ -- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled etc) -- ============================================================
pcall(function()
...
;(function()
    ...
    local spBtn=CategoryRefs.btnsSide["Speed"]; if spBtn then spBtn.TextColor3=C.white; spBtn.BackgroundTransparency=0.2; local i2=spBtn:FindFirstChild("indicator"); if i2 then i2.BackgroundTransparency=0.3 end end
end)()
...
;(function()
    ...
    -- Anti Ragdoll Toggle (nova versão simples)
...
print("ASTA REMAKE LOADED – Lagger corrigido, Reset rápido, Anti-ragdoll simples (forceReset) integrado.")
